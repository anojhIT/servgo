...
if(isset($_GET['action']) && isset($_GET['id'])) {
    ...
    header("Location: bookings.php?msg=" . urlencode("Booking $action successfully!"));
    exit();
}
...
$sql = "SELECT bookings.id, bookings.booking_date, bookings.status,
...
<div id="confirm-modal" style="display:none;position:fixed;inset:0;z-index:9999;background:rgba(10,10,20,0.6);backdrop-filter:blur(6px);align-items:center;justify-content:center;">
  <div style="background:#fff;border-radius:20px;padding:2.5rem 2rem;max-width:380px;width:90%;text-align:center;box-shadow:0 32px 80px rgba(0,0,0,0.35);animation:popIn 0.25s cubic-bezier(.22,1,.36,1);">
    ...
    <div style="display:flex;gap:0.75rem;">
      <button onclick="closeModal()" style="flex:1;padding:0.75rem;border-radius:10px;border:1.5px solid var(--border);background:#fff;font-family:inherit;font-size:0.875rem;font-weight:600;color:var(--muted);cursor:pointer;">Cancel</button>
      <a id="modal-btn" href="#" style="flex:1;padding:0.75rem;border-radius:10px;font-family:inherit;font-size:0.875rem;font-weight:700;cursor:pointer;text-decoration:none;display:flex;align-items:center;justify-content:center;"></a>
    </div>
  </div>
</div>
...
<div class="dash-wrap">
  ...
  <main class="dash-main">
    ...
    <div class="dash-header">
      <h1>All Bookings</h1>
      <p>Review and manage all customer service requests</p>
    </div>
    ...
    <div class="kpi-grid" style="margin-bottom:1.5rem;">
      ...
      <div class="kpi-card"><div class="kpi-label">Declined</div><div class="kpi-value"><?php echo $declined; ?></div></div>
    </div>

    <!-- Filter Tabs -->
    <div style="display:flex;gap:0.5rem;margin-bottom:1.25rem;flex-wrap:wrap;">
      <?php
        $tabs = [''=> 'All','Pending'=>'⏳ Pending','Accepted'=>'✅ Accepted','Declined'=>'❌ Declined','Completed'=>'🏁 Completed'];
        foreach($tabs as $val => $label):
          $active = ($filter===$val) ? 'background:var(--teal);color:var(--dark);border:1.5px solid var(--teal);' : 'background:#fff;color:var(--muted);border:1.5px solid var(--border);';
      ?>
        <a href="bookings.php<?php echo $val?'?filter='.$val:''; ?>"
           style="<?php echo $active; ?>padding:0.45rem 1rem;border-radius:999px;font-size:0.82rem;font-weight:600;text-decoration:none;display:inline-flex;align-items:center;gap:0.4rem;">
          <?php echo $label; ?>
          <?php if($val=='Pending' && $pending>0): ?><span style="background:#EF4444;color:#fff;border-radius:999px;padding:0.1rem 0.45rem;font-size:0.7rem;"><?php echo $pending; ?></span><?php endif; ?>
        </a>
      <?php endforeach; ?>
    </div>

    <div class="table-card">
      <div class="table-card-header"><span class="table-card-title">Booking Requests</span></div>
      <table>
        <thead>
          <tr><th>#</th><th>Customer</th><th>Service</th><th>Date</th><th>Amount</th><th>Status</th><th>Action</th></tr>
        </thead>
        <tbody>
            ...
            <td>
              ...
              <?php if($row['status'] == 'Pending'): ?>
                <button onclick="showModal(
                  'bookings.php?action=Accepted&id=<?php echo $row['id']; ?>',
                  '✅','Accept Booking',
                  'Accept booking from <?php echo htmlspecialchars($row["customer_name"]); ?>?',
                  'var(--teal)','var(--dark)','Accept'
                )" class="btn btn-success btn-sm">✅ Accept</button>
                <button onclick="showModal(
                  'bookings.php?action=Declined&id=<?php echo $row['id']; ?>',
                  '❌','Decline Booking',
                  'Decline this booking from <?php echo htmlspecialchars($row["customer_name"]); ?>?',
                  '#EF4444','#fff','Decline'
...
function showModal(url,icon,title,desc,btnBg,btnColor,btnText){
  ...
  btn.href=url; btn.style.background=btnBg; btn.style.color=btnColor; btn.textContent=btnText;
